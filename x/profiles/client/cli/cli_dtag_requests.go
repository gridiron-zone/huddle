...
func GetCmdRequestDTagTransfer() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`%s tx profiles request-dtag-transfer huddle13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud`, version.AppName),
...
func GetCmdCancelDTagTransfer() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`%s tx profiles cancel-dtag-transfer-request huddle13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud`, version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
If this happens, your DTag and the other user's one will be effectively swapped.`,
		Example: fmt.Sprintf(`%s tx profiles accept-dtag-transfer-request "leoDiCaprio" huddle13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud`, version.AppName),
...
func GetCmdRefuseDTagTransfer() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(`%s tx profiles refuse-dtag-transfer-request huddle13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud`, version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
%s tx profiles incoming-dtag-transfer-requests huddle13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud
