...
```bash
huddle query profiles profile [address_or_dtag] [flags]
```

Example: 
```bash
huddle query profiles profile jabbey
```

Example Output: 
```yaml
profile:
  '@type': /huddle.profiles.v2.Profile
  account:
    '@type': /cosmos.auth.v1beta1.BaseAccount
    account_number: "203491"
    address: huddle1xwazl8ftks4gn00y5x3c47auquc62ssu3nt23g
    pub_key:
...
```bash
huddle query profiles incoming-dtag-transfer-requests [[receiver]] [flags]
```

Example:
```bash
huddle query profiles incoming-dtag-transfer-requests
```
...
requests:
- dtag_to_trade: Jack
  receiver: huddle13yp2fq3tslq6mmtq4628q38xzj75ethzela9uu
  sender: huddle1tamzg6rfj9wlmqhthgfmn9awq0d8ssgfr8fjns
```
...
```bash
huddle query profiles chain-links [[user]] [[chain_name]] [[target]] [flags]
```
...
Example: 
```bash
huddle query profiles chain-links huddle16c60y8t8vra27zjg2arlcd58dck9cwn7p6fwtd cosmos 
```
...
links:
- address:
    '@type': /huddle.profiles.v2.Bech32Address
    prefix: cosmos
  ...
  user: huddle16c60y8t8vra27zjg2arlcd58dck9cwn7p6fwtd
pagination:
  next_key: null
  total: "0"
```
...
```bash
huddle query profiles default-external-addresses [[owner]] [[chain_name]] [flags]
```
...
Example:
```bash
huddle query profiles default-external-address huddle1evj20rymftvecmgn8t0xv700wkjlgucwfy4f0c cosmos
```
...
```bash
huddle query profiles app-links [[user]] [[application]] [[username]] [flags]
```
...
Example: 
```bash
huddle query profiles app-links huddle16c60y8t8vra27zjg2arlcd58dck9cwn7p6fwtd twitter
```
...
- creation_time: "2021-11-04T10:50:54.794047348Z"
  ...
  oracle_request:
    ...
    client_id: huddle16c60y8t8vra27zjg2arlcd58dck9cwn7p6fwtd-twitter-lucagraziotti
    id: "537363"
    oracle_script_id: "32"
  result:
    success:
...
```bash
huddle query profiles parameters [flags]
```

Example:
```bash
huddle query profiles parameters
```
...
```bash
huddle tx profiles --help
```

#### save
The `save` command allows users to save their Huddle profile. 

```bash
huddle tx profiles save [flags]
```

Example:
```bash
huddle tx profiles save 
 	--dtag "JohnSnow" \
 	--nickname "John Snow" \
...
#### delete
The `delete` command allows users to delete their Huddle profile. 

```bash
huddle tx profiles delete [flags]
```

Example:
```bash
huddle tx profiles delete
```
...
```bash
huddle tx profiles request-dtag-transfer [address] [flags]
```

Example:
```bash
huddle tx profiles request-dtag-transfer huddle13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud
```
...
```bash
huddle tx profiles cancel-dtag-transfer-request [recipient] [flags]
```

Example: 
```bash
huddle tx profiles cancel-dtag-transfer-request huddle13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud
```
...
```bash
huddle tx profiles accept-dtag-transfer-request [DTag] [address] [flags]
```

Example:
```bash
huddle tx profiles accept-dtag-transfer-request "NewJohnSnow" huddle13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud
```
...
```bash
huddle tx profiles refuse-dtag-transfer-request [sender] [flags]
```

Example:
```bash
huddle tx profiles refuse-dtag-transfer-request huddle13p5pamrljhza3fp4es5m3llgmnde5fzcpq6nud
```

#### link-chain 
The `link-chain` command allows users to link a new chain account to their Huddle profile. 

```bash
huddle tx profiles link-chain [data-file] [flags]
```

Example:
```bash
huddle tx profiles link-chain chain-link.json
```
...
Example: 
```bash
huddle tx profiles unlink-chain "cosmos" cosmos18xnmlzqrqr6zt526pnczxe65zk3f4xgmndpxn2`
```
...
Example:
```bash
huddle tx profiles set-default-external-address "cosmos" cosmos18xnmlzqrqr6zt526pnczxe65zk3f4xgmndpxn2
```

#### link-app
The `link-app` command allows users to link an external app account to their Huddle profile. 

```bash
huddle tx profiles link-app [src-port] [src-channel] [application] [username] [verification-call-data] [flags]
```

Example: 
```bash
huddle tx profiles link-app "profiles" "channel-0" "twitter" "twitter_user" "7B22757365726E616D65223A22526963636172"
```
...
```bash
huddle tx profiles unlink-app [application] [username] [flags]
```

Example:
```bash
huddle tx profiles unlink-app "twitter" "twitter_user"
```
...
```bash
huddle.profiles.v2.Query/Profile
```

Example:
```bash
grpcurl -plaintext \
  -d '{"user": "jack"}' localhost:9090 huddle.profiles.v2.Query/Profile
```
...
```json
{
  "profile": {
    ...
    "account": {
      "@type": "/cosmos.auth.v1beta1.BaseAccount",
      "address": "huddle13yp2fq3tslq6mmtq4628q38xzj75ethzela9uu",
      "pubKey": {
        "@type": "/cosmos.crypto.secp256k1.PubKey",
...
```bash
huddle.profiles.v2.Query/IncomingDTagTransferRequests
```

Example: 
```bash
grpcurl -plaintext \
  -d '{"receiver": "huddle13yp2fq3tslq6mmtq4628q38xzj75ethzela9uu"}' localhost:9090 huddle.profiles.v2.Query/IncomingDTagTransferRequests
```
...
```bash
huddle.profiles.v2.Query/ChainLinks
```
...
```bash
grpcurl -plaintext \
  -d '{"user": "huddle16c60y8t8vra27zjg2arlcd58dck9cwn7p6fwtd", "chain_name": "osmosis"}' localhost:9090 huddle.profiles.v2.Query/ChainLinks
```
...
```bash
huddle.profiles.v2.Query/ChainLinkOwners
```
...
```bash
grpcurl -plaintext \
  -d '{"chain_name": "osmosis", "target": "osmo1kdl9888p5ez2mt7w59anugx7z0ek927gvdvk7p"}' localhost:9090 huddle.profiles.v2.Query/ChainLinkOwners
```
...
```bash
huddle.profiles.v2.Query/huddle.profiles.v3.Query/DefaultExternalAddresses
```
...
```bash
grpcurl -plaintext  \
  -d '{"owner": "huddle1evj20rymftvecmgn8t0xv700wkjlgucwfy4f0c", "chainName": "cosmos"}' localhost:9090 huddle.profiles.v3.Query/DefaultExternalAddresses
```
...
```json
{
  "links": [
    {
      "user": "huddle1evj20rymftvecmgn8t0xv700wkjlgucwfy4f0c",
      "address": {"@type":"/huddle.profiles.v3.Bech32Address","prefix":"cosmos","value":"cosmos13n9wek2ktpxhpgfrd39zlaqaeahxuyusxrsfvn"},
      "proof": {
        "pubKey": {"@type":"/cosmos.crypto.secp256k1.PubKey","key":"AqYZhHKaeBcrYktZEvor/SUDlHCkv5JBplaG2vc2bvfS"},
        "signature": {"@type":"/huddle.profiles.v3.SingleSignatureData","mode":"SIGN_MODE_DIRECT","signature":"3yCU8/HKv7Vn0sf7HB+AhV/hK37DCBAkQkXdruaFSvMjRZB1XrYkpKWZVi+xnhSenc1p951Q1058rrYjuNCk9g=="},
...
```bash
huddle.profiles.v2.Query/ApplicationLinks
```
...
```bash
grpcurl -plaintext \
  -d '{"user": "huddle16c60y8t8vra27zjg2arlcd58dck9cwn7p6fwtd", "application": "twitter"}' localhost:9090 huddle.profiles.v2.Query/ApplicationLinks
```
  ...
  "links": [
    {
      ...
      "oracleRequest": {
        ...
        "clientId": "huddle16c60y8t8vra27zjg2arlcd58dck9cwn7p6fwtd-twitter-lucagraziotti"
      },
      "result": {
        "success": {
          "value": "6c7563616772617a696f747469",
...
```bash
huddle.profiles.v2.Query/ApplicationLinkByClientID
```

Example:
```bash
grpcurl -plaintext \
  -d '{"client_id": "huddle16c60y8t8vra27zjg2arlcd58dck9cwn7p6fwtd-twitter-lucagraziotti"}' localhost:9090 huddle.profiles.v2.Query/ApplicationLinkByClientID
```
...
```bash
huddle.profiles.v2.Query/ApplicationLinkOwners
```
...
```bash
grpcurl -plaintext \
  -d '{"application": "twitter", "user": "lucagraziotti"}' localhost:9090 huddle.profiles.v2.Query/ApplicationLinkOwners
```
...
```bash
huddle.profiles.v2.Query/Params
```

Example:
```bash
grpcurl localhost:9090 huddle.profiles.v2.Query/Params
```
...
### Profile
The `profile` endpoint allows users to query for a Huddle profile using a DTag or an address. 

```
/huddle/profiles/v2/profiles/{DTag or address}
```
...
```
/huddle/profiles/v2/dtag-transfer-requests?receiver={address}
```
...
```
/huddle/profiles/v2/chain-links?user={user}&chain_name={chainName}&target={target}
```
...
```
/huddle/profiles/v2/chain-links/owners?chain_name={chainName}&target={target}
```
...
```
/huddle/profiles/v3/default-external-addresses?owner={owner}&chain_name={chainName}
```
...
```
/huddle/profiles/v2/app-links?user={user}&application={application}&username={username}
```
...
```
/huddle/profiles/v2/app-links/clients/{client_id}
```
...
```
/huddle/profiles/v2/app-links/owners?application={applicationName}&username={username}
```
