...
- Create a `~/.tmkms/tmkms.toml` file with the following content (use an adequate `chain_id`)
   ...
   chain_id = "huddle-mainnet-1"
   reconnect = true # true is the default
   secret_key = "~/.tmkms/secret_connection.key"
  
   [[providers.ledgertm]]
   chain_ids = ["huddle-mainnet-1"]
   ```

- Edit `addr` to point to your `huddle` instance.
- Adjust `chain-id` to match your `.huddle/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `huddle`.
...
07:28:24 [INFO] [keyring:ledgertm:ledgertm] added validator key huddlevalconspub1zcjduepqepu8acj4qua576zzquvcly2un0xkzhwh0ehvgmx8gxgl34zhkceskthfp6
07:28:24 [INFO] KMS node ID: 1BC12314E2E1C29015B66017A397F170C6ECDE4A
```

The KMS may complain that it cannot connect to huddle. That is fine, we will fix it in the next section.

This output indicates the validator key linked to this particular device is: `huddlevalconspub1zcjduepqepu8acj4qua576zzquvcly2un0xkzhwh0ehvgmx8gxgl34zhkceskthfp6`

Take note of the validator pubkey that appears in your screen. *We will use it in the next section.*

## Huddle configuration
You need to enable KMS access by editing `.huddle/config/config.toml`. In this file, modify `priv_validator_laddr` 
to create a listening address/port or a unix socket in `huddle`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell huddle the key that we've got in the previous section.

```bash
huddle gentx --name kmsval --pubkey {.ValidatorKey} 
```

Now start `huddle`. You should see that the KMS connects and receives a signature request.
