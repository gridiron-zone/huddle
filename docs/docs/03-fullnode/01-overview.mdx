...
Inside Huddle, there are various types of pruning strategies that can be applied. The main ones are:
...
In order to run a fullnode, you need to build `huddle` which requires `Go`, `git`, `gcc` and `make` installed.
...
Once you have set up your environment correctly, you are now ready to install the Huddle software and start your full
node.
